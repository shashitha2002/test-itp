...
const Tables = ({ userdata, deleteUser, userGet, handlePrevious, handleNext, page, pageCount, setPage }) => {
  ...
  const handleChange = async (id, status) => {
    ...
    if (response.status === 200) {
      userGet();
      toast.success("UserType Updated")
    } else {
      toast.error("error ")
    }
  }
                            ...
                            <td className='d-flex align-items-center'>
                              <Dropdown className='text-center'>
                                <Dropdown.Toggle className='dropdown_btn' id="dropdown-basic">
                                  <Badge bg={element.status == "wholesale" ? "primary" : "danger"}>
                                    {element.status} <i class="fa-solid fa-angle-down"></i>
                                  </Badge>
                                </Dropdown.Toggle>
                                <Dropdown.Menu>
                                  <Dropdown.Item onClick={() => handleChange(element._id, "wholesale")}>Wholesale Buyer</Dropdown.Item>
                                  <Dropdown.Item onClick={() => handleChange(element._id, "supplier")}>Supplier</Dropdown.Item>
                                  <Dropdown.Item onClick={() => handleChange(element._id, "regular")}>Regular Customer</Dropdown.Item>
